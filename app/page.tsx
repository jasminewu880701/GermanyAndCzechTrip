...
export default function TravelApp() {
  ...
  const [itineraries, setItineraries] = useState<any>({
    "2026-02-14": { events: [] },
    "2026-02-15": { acc: { name: "Staycity Frankfurt Airport", addr: "Amelia-Mary-Earhart-Straße 9, Frankfurt" }, events: [] }
  });

  const currentDay = itineraries[selectedDate] || { events: [] };

  // 1. 【讀取】當切換日期時，從 Supabase 抓取資料
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (data && data.events) {
        setItineraries((prev: any) => ({
          ...prev,
          [selectedDate]: { ...prev[selectedDate], events: data.events }
        }));
      }
    };
    fetchData();
  ...
  // 2. 【儲存】核心儲存函數
  const saveToDB = async (updatedEvents: any[]) => {
  ...
  return (
    <div className="min-h-screen bg-[#FFFAF0] text-[#333] pb-32 font-sans">
            ...
            <div key={item.id} className="relative">
              {editingId === item.id ? (
                <div className="bg-white p-6 rounded-3xl shadow-xl space-y-4 border border-orange-200">
                  ...
                  <button onClick={() => handleDelete(item.id)} className="w-full py-4 mt-2 text-red-500 font-bold flex items-center justify-center gap-2 bg-red-50 rounded-2xl"><Trash2 className="w-5 h-5"/> 刪除此行程</button>
                </div>
              ) : (
                <div className="bg-white p-6 rounded-[32px] shadow-sm active:scale-95 transition-transform" onClick={() => setEditingId(item.id)}>
                  <div className="flex items-center gap-3 mb-3">{getIcon(item.type)}<span className="text-lg font-black text-orange-600">{item.time}</span></div>
                  ...
                  <div className="bg-[#FFF9F2] p-5 rounded-2xl mb-4"><p className="text-base text-gray-600 leading-relaxed whitespace-pre-line">{item.note}</p></div>
                  {item.info && (
                  ...
                  <div className="flex justify-end"><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.location)}`} target="_blank" onClick={(e) => e.stopPropagation()} className="bg-orange-500 text-white px-8 py-4 rounded-2xl text-base font-black shadow-lg flex items-center gap-2"><Navigation className="w-5 h-5"/> 導航</a></div>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex gap-4 bg-[#2D241E]/95 backdrop-blur-xl p-4 rounded-[40px] shadow-2xl border border-white/10 z-[100]">
        <button onClick={() => handleAdd('transport')} className="flex flex-col items-center gap-1 px-4"><div className="bg-blue-500 p-3.5 rounded-full text-white shadow-lg"><Car className="w-7 h-7"/></div><span className="text-[11px] text-white font-bold">交通</span></button>
        <button onClick={() => handleAdd('spot')} className="flex flex-col items-center gap-1 px-4"><div className="bg-orange-500 p-3.5 rounded-full text-white shadow-lg"><Castle className="w-7 h-7"/></div><span className="text-[11px] text-white font-bold">景點</span></button>
        <button onClick={() => handleAdd('food')} className="flex flex-col items-center gap-1 px-4"><div className="bg-green-500 p-3.5 rounded-full text-white shadow-lg"><UtensilsCrossed className="w-7 h-7"/></div><span className="text-[11px] text-white font-bold">飲食</span></button>
      </div>
    </div>
  );
}